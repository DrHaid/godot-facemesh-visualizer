...
def main():
	...
	parser.add_argument('-p', '--preview', action='store_true', dest='preview', help='Open preview window with visualized face mesh')
	args = parser.parse_args()

	face_mesh_capture = FaceMeshCapture()
	socket_connection = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)

	cam = cv2.VideoCapture('webcam.mp4') if args.mock else cv2.VideoCapture(args.cam_index)
	while True:
